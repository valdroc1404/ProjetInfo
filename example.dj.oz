...
local
   ...
   Gauche2 = [instrument(nom:voice [etirer(facteur:0.5 [etirer(facteur:0.8 c#3) etirer(facteur:1.5 c#3) etirer(facteur:3.9 c#3)])])]  
   ...
   Droite1 = [partition(muet([etirer(facteur:0.5 a)])) partition(A1) partition(B1) repetition(nombre:4 partition(C1)) partition(A1) partition(B1) repetition(nombre:3 partition(C1)) partition(etirer(facteur:1.55 g#3))]
   ...
   Animaux = repetition(nombre:10 [wave('wave/animaux/frogs.wav')])
in

   
%%%%%%%%%%%%%%%%% Partition finale %%%%%%%%%%%%%%%%%
 
   fondu(ouverture:4.0 fermeture:4.0 merge([0.6#Gauche 0.1#Droite2 0.1#Droite1 0.1#Droite3 0.1#Animaux]))
